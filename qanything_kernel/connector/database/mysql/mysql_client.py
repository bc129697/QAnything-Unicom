...
class KnowledgeBaseManager:
    ...
    def create_tables_(self):
        ...
        query = """
            CREATE TABLE IF NOT EXISTS KnowledgeBase (
                ...
                kb_metadata LONGTEXT
            );

        """
        # 针对已部署的数据库，新增后三个字段
        # alter_query = "ALTER TABLE KnowledgeBase ADD COLUMN creation_time TIMESTAMP, ADD COLUMN parser_config LONGTEXT, ADD COLUMN kb_metadata LONGTEXT;"
        # self.execute_query_(alter_query, (), commit=True)
        # debug_logger.info(f"执行{alter_query}成功")
        ...
        # 针对已部署的数据库，file_metadata字段
        # alter_query = "ALTER TABLE File ADD COLUMN file_metadata TEXT;"
        # self.execute_query_(alter_query, (), commit=True)
        # debug_logger.info(f"执行{alter_query}成功")
        ...
        # 针对已部署的数据库，需要i修噶answer字段长度
        # alter_query = "ALTER TABLE Faqs MODIFY COLUMN answer VARCHAR(8192) NOT NULL;"
        # self.execute_query_(alter_query, (), commit=True)
        # debug_logger.info("执行ALTER TABLE Faqs MODIFY COLUMN answer VARCHAR(8192) NOT NULL;")
    ...
    def check_kb_exist(self, user_id, kb_ids):
        ...
        query = "SELECT kb_id, user_id FROM KnowledgeBase WHERE kb_id IN ({}) AND deleted = 0".format(
            kb_ids_str) # 判断所有知识库中，存在的知识库
        ...
        valid_kb_ids_userid = {kb_info[0]: kb_info[1] for kb_info in result}
        unvalid_kb_ids = []
        for kb_id in kb_ids:
            if kb_id in valid_kb_ids_userid.keys():
                if valid_kb_ids_userid[kb_id] != user_id:
                    unvalid_kb_ids.append([kb_id, "Belongs to other users"]) # 知识库存在，但是不属于该用户
            else:
                unvalid_kb_ids.append([kb_id, "not_exist"])
    ...
    def check_file_exist_byuserid(self, user_id, file_ids):
        # 筛选出有效的文件
        if not file_ids:
            debug_logger.info("check_file_exist: file_ids is empty")
            return []
        ...
        query = """SELECT file_id, status FROM File 
                 WHERE deleted = 0
                 AND file_id IN ({})
                 AND user_id = %s""".format(file_ids_str)
        result = self.execute_query_(query, [user_id], fetch=True)
        debug_logger.info("check_file_exist {}".format(result))
        return result
    ...
    def new_milvus_base(self, kb_id, user_id, kb_name, parser_config, user_name=None):
        if not self.check_user_exist_(user_id):
            self.add_user_(user_id, user_name)
        import time
        create_timestamp = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
        parser_config = json.dumps(parser_config, ensure_ascii=False)
        query = "INSERT INTO KnowledgeBase (kb_id, user_id, kb_name, parser_config, creation_time) VALUES (%s, %s, %s, %s, %s)"
        try:
            self.execute_query_(query, (kb_id, user_id, kb_name, parser_config, create_timestamp), commit=True)
    ...
    def get_knowledge_bases(self, user_id):
        # 只获取后缀为KB_SUFFIX的知识库
        query = (f"SELECT kb_id, kb_name, creation_time, parser_config, kb_metadata FROM KnowledgeBase WHERE user_id = %s AND deleted = 0 AND "
    ...
    def delete_knowledge_base(self, user_id, kb_ids):
        ...
        query = "DELETE FROM KnowledgeBase WHERE user_id = %s AND kb_id IN ({})".format(kb_ids_str)
        ...
        query = """DELETE FROM File WHERE kb_id IN ({}) AND kb_id IN (SELECT kb_id FROM KnowledgeBase WHERE user_id = %s)""".format(kb_ids_str)
        self.execute_query_(query, (user_id,), commit=True)
    ...
    def get_kbid_by_fileid(self, file_id):
        query = "SELECT kb_id FROM File WHERE file_id = %s"
        result = self.execute_query_(query, (file_id,), fetch=True)
        if result:
            return result[0][0]
        else:
    ...
    def get_files(self, user_id, kb_id, file_id=None, file_name=None, status=None, limit=100, offset=0):

        base_query = """
            SELECT file_id, file_name, status, file_size, content_length, timestamp,
                   file_location, file_url, chunk_size, msg, file_metadata
            FROM File
        ...
        if file_name is not None:
            base_query += f" AND file_name LIKE %s"
            params.append(f"%{file_name}%")  # 前后加 % 实现“包含”
    ...
    def get_files_count(self, user_id, kb_id, status=None):
        
        base_query = """
            SELECT COUNT(*) as file_count
            FROM File
            WHERE kb_id = %s AND user_id = %s AND deleted = 0
        """
        ...
        file_count = self.execute_query_(base_query, params, fetch=True)
        return file_count
    ...
    def delete_files(self, kb_id, file_ids):
        ...
        query = "DELETE FROM File WHERE kb_id = %s AND file_id IN ({})".format(file_ids_str)
    ...
    def update_jsondata(self, doc_id, json_data):
        json_data = json.dumps(json_data, ensure_ascii=False)
        query = "UPDATE Documents SET json_data = %s WHERE doc_id = %s"
        self.execute_query_(query, (json_data, doc_id), commit=True, check=True)

    def update_document(self, doc_id, update_content, faq_dict=None):
        ori_doc_json = self.get_document_by_doc_id(doc_id)
        ori_doc_json['kwargs']['page_content'] = update_content
        if faq_dict:
            ori_doc_json['kwargs']['metadata']['faq_dict'] = faq_dict
            file_name = f'FAQ_{faq_dict.get("question", "unknown")}.faq'
            ori_doc_json['kwargs']['metadata']['file_name'] = file_name
            ori_doc_json['kwargs']['metadata']['headers']['文件名'] = file_name
    ...
    def update_faq(self, user_id, faq_id, question, answer):
        query = "UPDATE Faqs SET question = %s, answer = %s WHERE faq_id = %s AND user_id = %s"
        self.execute_query_(query, (question, answer, faq_id, user_id), commit=True)

        query = "UPDATE File SET file_name = %s WHERE file_id = %s"
        file_name = f"FAQ_{question}.faq"
        self.execute_query_(query, (file_name, faq_id), commit=True)
    ...
    def get_document_by_file_id(self, file_id, batch_size=100) -> Optional[List]:
        ...
        query = "SELECT doc_id, json_data FROM Documents WHERE doc_id REGEXP %s"
        offset = 0
        ...
        while True:
            ...
            doc_all = self.execute_query_(paginated_query, (f"^{file_id}_\\d+$", batch_size, offset), fetch=True)
    ...
    def delete_documents(self, file_ids):
        ...
        for file_id in file_ids:
            ...
            query = f"SELECT doc_id FROM Documents WHERE doc_id REGEXP \"{file_id}_\\d+$\""
    ...
    def delete_docs(self, doc_ids):
        ...
        if doc_ids:
            ...
            for i in range(0, len(doc_ids), batch_size):
                ...
                res = self.execute_query_(delete_query, batch_doc_ids, commit=True, check=True)
                total_deleted += res
        debug_logger.info(f"Deleted docs count: {total_deleted}")
    ...
    def get_file_name(self, file_id):
        ...
        result = self.execute_query_(query, (file_id,), fetch=True)
        file_name = result[0][0] if result else None
        return file_name
    
    def update_kb_metadata(self, user_id, kb_id, json_data):
        json_data = json.dumps(json_data, ensure_ascii=False)
        query = "UPDATE KnowledgeBase SET kb_metadata = %s WHERE kb_id = %s AND user_id = %s"
        self.execute_query_(query, (json_data, kb_id, user_id), commit=True)
    
    def update_file_metadata(self, user_id, file_id, json_data):
        json_data = json.dumps(json_data, ensure_ascii=False)
        query = "UPDATE File SET file_metadata = %s WHERE file_id = %s AND user_id = %s"
        self.execute_query_(query, (json_data, file_id, user_id), commit=True)
        
    def move_file(self, user_id, file_id, target_kb_id):
        # 更新File表中的kb_id
        # 获取file_id的kb_id以及file_location
        query = "SELECT kb_id, file_location FROM File WHERE file_id = %s AND user_id = %s"
        result = self.execute_query_(query, (file_id, user_id), fetch=True)
        if not result:
            return
        source_kb_id, file_location = result[0]
        file_location = file_location.replace(source_kb_id, target_kb_id)
        debug_logger.info(f"mysql move_file: {file_id}, from {source_kb_id} to {target_kb_id}, new location: {file_location}")

        query = "UPDATE File SET kb_id = %s, file_location = %s WHERE file_id = %s AND user_id = %s"
        self.execute_query_(query, (target_kb_id, file_location, file_id, user_id), commit=True)

        # 文件可能是qa
        query = "UPDATE Faqs SET kb_id = %s WHERE faq_id = %s AND user_id = %s"
        self.execute_query_(query, (target_kb_id, file_id, user_id), commit=True)
        
        # 更新Document表中的kb_id
        file_documents = self.get_document_by_file_id(file_id)
        if not file_documents:
            return
        for json_data in file_documents:
            doc_id = json_data['kwargs']['chunk_id']
            json_data['kwargs']['metadata']['kb_id'] = target_kb_id
            self.update_jsondata( doc_id, json_data)
    
    def get_kb_parser_config(self, kb_id):
        query = "SELECT parser_config FROM KnowledgeBase WHERE kb_id = %s"
        result = self.execute_query_(query, (kb_id,), fetch=True)
        if result:
