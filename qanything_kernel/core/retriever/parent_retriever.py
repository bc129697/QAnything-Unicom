    ...
    async def aadd_documents(
        ...
        if self.parent_splitter is not None and not single_parent:
            ...
            for doc in documents:
                if doc.metadata['single_parent'] or doc.metadata['has_table'] or num_tokens_embed(doc.page_content) <= parent_chunk_size:
                    if need_split_docs:
        ...
        for idx, doc in enumerate(embed_docs):
            ...
            del doc.metadata['bboxes']
            del doc.metadata['single_parent']
        
        es_documents = copy.deepcopy(documents)
        for idx, doc in enumerate(es_documents):
            del doc.metadata['title_lst']
            del doc.metadata['has_table']
            del doc.metadata['images']
            del doc.metadata['file_name']
            ...
            del doc.metadata['bboxes']
            del doc.metadata['single_parent']
            doc.metadata[self.id_key] = doc_ids[idx]  # 重要变更，向ES中存入parent文档时，id使用doc_id字段
        ...
        if es_store is not None:
            try:
                ...
                # es_res = await es_store.aadd_documents(embed_docs, ids=doc_child_ids) # 重要变更，使用自动生成不重复的ids  
                es_res = await es_store.aadd_documents(es_documents, ids=doc_ids)  # 重要变更，向ES中存入parent文档
        ...
        return len(doc_ids), time_record
...
class ParentRetriever:
    def __init__(self, vectorstore_client: VectorStoreMilvusClient, mysql_client: KnowledgeBaseManager, es_client: StoreElasticSearchClient
        ...
        self.parent_chunk_size = chunk_size #DEFAULT_PARENT_CHUNK_SIZE
    ...
    async def insert_documents(self, docs, parent_chunk_size, single_parent=False):
        ...
        ids = None if not single_parent else [doc.metadata.get('doc_id', doc.metadata['file_id']+'_'+str(index)) for index,doc in enumerate(docs)]
    ...
    async def get_retrieved_documents(self, query: str, partition_keys: List[str], time_record: dict,
        ...
        for partition_key in partition_keys:
            if partition_key.startswith('KB'):
                kb_ids.append(partition_key)
            else:
                file_ids.append(partition_key)
        query_docs=[]
        if kb_ids:
            expr = f'kb_id in {kb_ids}'
            debug_logger.info(f"Searching by Milvus with expr: {expr}")
            self.retriever.set_search_kwargs("similarity", k=top_k, expr=expr)
            query_docs_kb = await self.retriever.aget_relevant_documents(query)
            query_docs.extend(query_docs_kb)
        if file_ids:
            expr = f'file_id in {file_ids}'
            debug_logger.info(f"Searching by Milvus with expr: {expr}")
            self.retriever.set_search_kwargs("similarity", k=top_k, expr=expr)
            query_docs_file = await self.retriever.aget_relevant_documents(query)
            query_docs.extend(query_docs_file)


        # expr = f'kb_id in {kb_ids} or file_id in {file_ids}'
        # debug_logger.info(f"Searching by Milvus with expr: {expr}")
        ...
        try:
            ...
            if kb_ids and file_ids:
                filter = {
                    "bool": {
                        "should": [
                            {"terms": {"metadata.kb_id.keyword": kb_ids}}, 
                            {"terms": {"metadata.file_id.keyword": file_ids}}
                        ]
                    }
                }
            elif kb_ids:
                filter = [{"terms": {"metadata.kb_id.keyword": kb_ids}}]
            elif file_ids:
                filter = [{"terms": {"metadata.file_id.keyword": file_ids}}]

            es_sub_docs = await self.es_store.asimilarity_search(query, k=top_k, filter=filter)
            debug_logger.info(f"ES returned {len(es_sub_docs)} sub documents")
