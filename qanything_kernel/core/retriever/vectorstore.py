...
from qanything_kernel.connector.embedding.embedding_for_online_client import GeneralEmbeddings
...
class SelfMilvus(Milvus):
    ...
    async def aadd_texts(
        ...
        if self._should_flush():
            ...
            self._milvus_flush()
            insert_logger.info(f"Flushed collection after insert.cost: {round(time.perf_counter() - flush_start, 2)}")
    ...
    def update_kb_id_for_file_id(self, file_id: str, new_kb_id: str):
        ...
        try:
            ...
            if not results:
                insert_logger.info(f"No records found with file_id = {file_id}")
                return []
            
            insert_logger.info(f"Found {len(results)} records to update")
            insert_logger.info(f"results0: {results[0]}")
            ...
            # 执行upsert操作
            if self.auto_id:
                insert_logger.info(f"Auto ID is enabled, handling delete and insert separately")
                # 提取主键ID用于删除
                primary_keys = [record[self._primary_field] for record in results]
                
                # 删除旧记录
                if primary_keys:
                    delete_expr = f"{self._primary_field} in {primary_keys}"
                    delete_result = self.col.delete(expr=delete_expr)
                    insert_logger.info(f"Deleted {delete_result.delete_count} records")
                
                # 构造新的数据（不包含主键，因为auto_id=True）
                fields_without_primary = [x for x in self.fields if x != self._primary_field]
                new_data = {field: [] for field in fields_without_primary}
                # 填充每个字段的值
                for record in results:
                    for field in fields_without_primary:
                        if field == "kb_id":
                            new_data[field].append(new_kb_id)  # 更新kb_id
                        else:
                            new_data[field].append(record[field])  # 保留原值
                
                # 构造插入数据（不包含主键字段）
                insert_data = [new_data[field] for field in fields_without_primary]
                # insert_logger.info(f"Insert data prepared with fields: {insert_data}")
                
                # 执行插入操作（系统会自动生成新的主键）
                insert_result = self.col.insert(insert_data)
                insert_logger.info(f"Inserted {insert_result.insert_count} records")
                
            else:
                insert_logger.info(f"Auto ID is disabled, performing upsert")
                # 获取字段列表
                fields = list(results[0].keys())
                insert_logger.info(f"Fields to update: {fields}")
                
                # 构造更新数据
                updated_values = {field: [] for field in fields}
                
                # 填充每个字段的值
                for record in results:
                    for field in fields:
                        if field == "kb_id":
                            updated_values[field].append(new_kb_id)  # 更新kb_id
                        else:
                            updated_values[field].append(record[field])  # 保留原值
                
                # 构造upsert数据
                upsert_data = [updated_values[field] for field in fields]
                upsert_result = self.col.upsert(upsert_data)
                insert_logger.info(f"Upserted {upsert_result.upsert_count} records")
            ...
            if self._should_flush():
                ...
                self._milvus_flush()
                insert_logger.info(f"Flushed collection after insert.cost: {round(time.perf_counter() - flush_start, 2)}")
            insert_logger.info(f"Successfully updated kb_id to {new_kb_id} for {len(results)} records with file_id = {file_id}")

            
        except Exception as e:
            insert_logger.error(f"Error updating kb_id for file_id {file_id}: {e}")
            raise e
    ...
    def copy_kb_id_for_file_id(self, file_id: str, new_kb_id: str):
        ...
        try:
            ...
            # 执行upsert操作
            upsert_result = self.col.insert(upsert_data)
            insert_logger.info(f"Upserted {upsert_result.upsert_count} records")
...
class VectorStoreMilvusClient:
    def __init__(self, parser_config=None):
        ...
        if parser_config:
            from qanything_kernel.configs.model_config import LOCAL_EMBED_SERVICE_URL, EMBEDDING_MODEL_NAME, EMBEDDING_API_KEY
            self.embedding_base_url = parser_config.get('embedding_base_url', LOCAL_EMBED_SERVICE_URL)
            self.embedding_model_name = parser_config.get('embedding_model_name', EMBEDDING_MODEL_NAME)
            self.embedding_api_key = parser_config.get('embedding_api_key', EMBEDDING_API_KEY)
        
        # 处理collection_name, 确保符合Milvus的命名规范,数字 (0-9)、字母 (a-z, A-Z)、下划线 (_)
        self.collection_name = 'qanything_'+self.embedding_model_name
        self.collection_name = re.sub(r"[^0-9a-zA-Z_]", "", self.collection_name)
        

        self.local_vectorstore: Milvus = SelfMilvus(
            embedding_function=GeneralEmbeddings(self.embedding_base_url, self.embedding_model_name, self.embedding_api_key),
        ...
        debug_logger.info(
            f'init vectorstore {self.host}, {self.collection_name}')
