...
class BingSearch(ABC):
    ...
    def run(self, question, top_n=3, detail=True):
        ...
        try:
            url = 'https://cn.bing.com/search?pglt=427&q=' + str(question) # + '&rn=' + str(top_n)
            headers = {
            ...
            # 提取搜索结果
            search_results = []
            for item in soup.select('.b_algo'):  # .b_algo 是 Bing 搜索结果的容器类名
                # 提取标题
                title_tag = item.select_one('h2 a')  # 定位到 <h2> 标签内的 <a> 标签
                ...
                # 提取描述
                description_tag = item.select_one('.b_caption p')  # 描述通常在 .b_caption p 标签内
                description = description_tag.get_text(strip=True) if description_tag else None

                # source
                source_tag = item.select_one('.b_attribution cite')  # 来源信息
...
if __name__ == '__main__':
    search = BingSearch()
    results = search.run('985大学有哪些?', 10, detail=True)
    print(json.dumps(results, ensure_ascii=False, indent=4))
else:
    from qanything_kernel.configs.model_config import SUPORT_WEBSEARCH_TOOLS
    if BingSearch.__name__ not in SUPORT_WEBSEARCH_TOOLS:
        SUPORT_WEBSEARCH_TOOLS.append(BingSearch.__name__)
