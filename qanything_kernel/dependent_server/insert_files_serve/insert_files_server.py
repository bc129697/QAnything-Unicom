...
from qanything_kernel.configs.model_config import INSERT_SERVER_PORT, INSERT_SERVER_WORKERS
...
from qanything_kernel.configs.model_config import MYSQL_HOST_LOCAL, MYSQL_PORT_LOCAL, \
    MYSQL_USER_LOCAL, MYSQL_PASSWORD_LOCAL, MYSQL_DATABASE_LOCAL, MAX_CHARS, RAG_SERVER_PORT
import asyncio
import traceback
import time
import random
...
async def process_data(mysql_client, file_info, kb_name, time_record):    # async def process_data(retriever, milvus_kb, mysql_client, file_info, kb_name, time_record):
    ...
    parser_outline_timeout_seconds = 300
    parser_summary_timeout_seconds = 300
    content_length = -1
    status = 'green'
    ...
    # 根据解析要求，解析大纲和摘要
    if upload_infos:
        upload_infos = json.loads(upload_infos)
        parser_outline = upload_infos.get('parser_outline', False)
        parser_summary = upload_infos.get('parser_summary', False)
        ...
        try:
            if parser_outline:
                start = time.perf_counter()
                await asyncio.wait_for(
                    asyncio.to_thread(local_file.parser_outline),
                    timeout=parser_outline_timeout_seconds)
                parse_time = time.perf_counter()
                time_record['parse_outline'] = round(parse_time - start, 2)
                insert_logger.info(f'parse_outline text: {local_file.outline}')
                ...
                data = {"user_id": user_id.split('__')[0],
                        ...
                        "chunk_datas": [local_file.outline]}
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(f"http://127.0.0.1:{RAG_SERVER_PORT}/api/rag/chunk_embedding", json=data, headers=headers) as response:
                        if response.status == 200 and (await response.json())['code'] == 200:
                            insert_logger.info(f'outline request success')
                        else:
                            msg = f'outline request failed with status {response.status}'
                            insert_logger.error(f'{msg}')
                            return "red", content_length, chunks_number, msg
                insert_logger.info(f'parse_outline time: {parse_time - start}')
        except asyncio.TimeoutError:
            local_file.event.set()
            insert_logger.error(f'Timeout: parse_outline took longer than {parser_outline_timeout_seconds} seconds')
            status = 'red'
            time_record['parse_outline_timeout'] = True
            msg = f"parse_outline timeout: {parser_outline_timeout_seconds}s"
            return status, content_length, chunks_number, msg
        except Exception as e:
            error_info = f'parse_outline error: {traceback.format_exc()}'
            insert_logger.error(error_info)
            status = 'red'
            time_record['parse_outline_error'] = True
            msg = f"parse_outline error"
            return status, content_length, chunks_number, msg
        ...
        try:
            if parser_summary:
                start = time.perf_counter()
                await asyncio.wait_for(
                    asyncio.to_thread(local_file.parser_summary),
                    timeout=parser_summary_timeout_seconds)
                parse_time = time.perf_counter()
                time_record['parse_summary'] = round(parse_time - start, 2)
                insert_logger.info(f'parse_summary text: {local_file.summary}')
                ...
                data = {"user_id": user_id.split('__')[0],
                        ...
                        "chunk_datas": [local_file.summary]}
                headers = {"content-type": "application/json"}
                async with aiohttp.ClientSession() as session:
                    async with session.post(f"http://127.0.0.1:{RAG_SERVER_PORT}/api/rag/chunk_embedding", json=data, headers=headers) as response:
                        if response.status == 200 and (await response.json())['code'] == 200:
                            insert_logger.info(f'summary request success')
                        else:
                            msg = f'summary request failed with status {response.status}'
                            insert_logger.error(f'{msg}')
                            return "red", content_length, chunks_number, msg
                insert_logger.info(f'parse_summary time: {parse_time - start}')
        except asyncio.TimeoutError:
            local_file.event.set()
            insert_logger.error(f'Timeout: parse_summary took longer than {parser_summary_timeout_seconds} seconds')
            status = 'red'
            time_record['parse_summary_timeout'] = True
            msg = f"parse_summary timeout: {parser_summary_timeout_seconds}s"
            return status, content_length, chunks_number, msg
        except Exception as e:
            error_info = f'parse_summary error: {traceback.format_exc()}'
            insert_logger.error(error_info)
            status = 'red'
            time_record['parse_summary_error'] = True
            msg = f"parse_summary error"
            return status, content_length, chunks_number, msg
        
        mysql_client.update_file_msg(file_id, f'Processing:{random.randint(75, 90)}%')
    

    # 做embedding并插入到Milvus、ES
    try:
        start = time.perf_counter()
        single_parent = False
        chunks_number, insert_time_record = await asyncio.wait_for(
            retriever.insert_documents(local_file.docs, chunk_size, single_parent),
...
async def check_and_process(pool):
    ...
    while True:
        ...
        minutes = int(int(time.strftime("%M", time.localtime())) / INSERT_SERVER_WORKERS)
        dynamic_worker_id = (worker_id + minutes) % INSERT_SERVER_WORKERS
        id = None
        try:
            async with pool.acquire() as conn:  # 获取连接
                async with conn.cursor() as cur:  # 创建游标
                    ...
                    await cur.execute(query, (INSERT_SERVER_WORKERS, dynamic_worker_id))
                    ...
                    if file_to_update:
                        ...
                        await cur.execute(
                            "SELECT kb_name FROM KnowledgeBase WHERE kb_id=%s", (file_info[4],))
                        kb_name = await cur.fetchone()
                        

                        time_record = {}
                        # 现在处理数据
                        status, content_length, chunks_number, msg = await process_data(mysql_client, file_info, kb_name[0], time_record)
                        ...
                        sleep_time = 0.1
                        # if time_record.get('upload_total_time', 10) < 1:    # zhendui qa linshi jiejue
                        #     sleep_time = 10
                    else:
                        await conn.commit()
        except Exception as e:
...
async def setup_workers(app, loop):
    # 连接上milvus，先判断是否有这个collection，如果没有就创建
    tmp_mysql_client = KnowledgeBaseManager()
    del tmp_mysql_client
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=INSERT_SERVER_PORT, workers=INSERT_SERVER_WORKERS, access_log=False)
