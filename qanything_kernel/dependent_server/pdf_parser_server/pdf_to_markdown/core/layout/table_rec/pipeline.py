...
def html_table_to_markdown(html):
    ...
    for tr in table.find_all('tr'):
        ...
        for td in tr.find_all(['td', 'th']):
            ...
            cells.append({'text': td.text.strip(), 'colspan': colspan, 'rowspan': rowspan})
        grid.append(cells)
        max_cols = max(max_cols, sum(c['colspan'] for c in cells))

    # 扩展网格以处理 rowspan/colspan
    data = [[''] * max_cols for _ in range(len(grid))]
        ...
        for cell in row:
            ...
            for r in range(i, i + cell['rowspan']):
                for c in range(col, col + cell['colspan']):
                    ...
                    # data[r][c] = cell['text'] if r == i and c == col else '▌'  # 占位符
                    data[r][c] = cell['text']

    # 生成Markdown
    markdown = []
    headers = data[0]
    markdown.append('| ' + ' | '.join([cell.replace('▌', ' ') for cell in headers]) + ' |')
...
def html2markdown(html_text):
    ...
    # logging.info(f"html text: {html_text}")
    # logging.info(f"Markdown text: {markdown_text}")
    markdown_text = html_table_to_markdown(html_text)
