...
from qanything_kernel.dependent_server.pdf_parser_server.pdf_to_markdown.core.vision import Recognizer, LayoutRecognizer, \
    TableStructureRecognizer_LORE, LayoutRecognizer4YOLOv10
...
# from qanything_kernel.core.retriever.general_document import get_ocr_result_sync
from qanything_kernel.configs.model_config import PDF_MODEL_PATH, LOCAL_OCR_SERVICE_URL
...
class HuParser:
    def __init__(self, device=torch.device("cpu")):
        ...
        if hasattr(self, "model_speciess"):
            ...
            logging.info("Using LayoutRecognizer4YOLOv10")
            self.layouter: LayoutRecognizer4YOLOv10 = LayoutRecognizer4YOLOv10("layout", device) 
        self.tbl_det = TableStructureRecognizer_LORE(device)
    ...
    def __ocr(self, pagenum, img, chars, ZM=3):
        ...
        bxs = get_ocr_result_sync(img_data, "ocr_detect")
        if not bxs:
            self.boxes.append([])
            return
        # 筛选box， 针对斜着的框，直接进行删除，大概率是水印
        def is_rotated(bbox):
            # 提取四个点的坐标
            ...
            # 计算角度
            angle1 = math.degrees(math.atan2(points[3][1] - points[0][1], points[3][0] - points[0][0]))
            angle2 = math.degrees(math.atan2(points[2][1] - points[1][1], points[2][0] - points[1][0]))
            
            # 判断对角线长度是否接近以及角度是否接近
            if abs(diag1 - diag2) < 5 and abs(abs(angle1) - 90) < 5 and abs(abs(angle2) - 90) < 5:
                return False
            else:
                return True
        bxs = [[b,t] for b,t  in bxs if not is_rotated(b)]
        ...
        for b in bxs:
            if not b["text"]:
                ...
                bbox=[[left, top], [right, top], [right, bott], [left, bott]]
                img_data["bbox"] = bbox
                b["text"] = get_ocr_result_sync(img_data, api="ocr_recognizer")
    ...
    def _layouts_rec(self, ZM, drop=True):
        ...
        self.boxes, self.page_layout = self.layouter(
            self.page_images, self.boxes, ZM, thr=0.3, drop=drop)
        
        # insert_logger.info(f"page layout: {self.page_layout}")
        # insert_logger.info(f"boxes: {self.boxes}")

        # layouts = self.layouter.layouts(self.page_images, thr=0.15)
        ...
        def draw_box(im, result, lables, threshold=0.5):
            ...
            for dt in result:
                ...
                xmin, ymin, xmax, ymax = dt["x0"]*ZM, dt["top"]*ZM, dt["x1"]*ZM, dt["bottom"]*ZM #dt["bbox"]
                draw.line(
        ...
        for i, lyt in enumerate(self.page_layout):
            img = self.page_images[i].copy()
            img = draw_box(img, lyt, self.layouter.labels, 0.15)
            img.save(self.filename.replace(".pdf", "_{}.png".format(i)), quality=95)
            logging.info("save result to: " + self.filename.replace(".pdf", "_{}.png".format(i)))
    ...
    def _extract_table_figure(self, need_image, ZM,
        ...
        merge_header = False
        table_header = ''
        logging.debug("deal Tables: %d" % len(tables))
        for k, bxs in tqdm(tables.items()):
            if not bxs:
                continue
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        for i, page in enumerate(self.pdf):
            ...
            self.ocr_res.append(page_ocr_res)   #未使用模型进行ocr识别
        ...
        for i, img in tqdm(enumerate(self.page_images)):
            ...
            self.__ocr_pdf(i + 1, self.ocr_res[i], zoomin)  #对box进行排序，删除空内容
            if callback:
    ...
    def __images2__(self, fnm, zoomin=3, page_from=0,
                   page_to=299, callback=None):
        self.lefted_chars = []
        self.mean_height = []
        self.mean_width = []
        ...
        try:
            ...
            #                     enumerate(self.pdf.pages[page_from:page_to])]
            try:
                self.page_chars = [[{**c, 'top': c['top'], 'bottom': c['bottom']} for c in page.dedupe_chars().chars if self._has_color(c) and math.atan2(c['matrix'][1],c['matrix'][0])<0.1] 
