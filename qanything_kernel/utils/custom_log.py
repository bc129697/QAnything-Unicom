...
milvus_log_folder = './logs/milvus_logs'
# 确保日志文件夹存在
...
if not os.path.exists(milvus_log_folder):
    os.makedirs(milvus_log_folder)
...
milvus_logger = logging.getLogger('milvus_logger')
...
milvus_handler = ConcurrentRotatingFileHandler(os.path.join(milvus_log_folder, "milvus_api.log"), "a", 64 * 1024 * 1024, 256)

# 创建一个带有自定义字段的格式器
formatter = logging.Formatter(f"%(asctime)s - [Function: %(funcName)s] - %(levelname)s - %(message)s")

# 设置日志格式
milvus_handler.setFormatter(formatter)

# 将 handler 添加到 logger 中，这样 logger 就可以使用这个 handler 来记录日志了
milvus_logger.addHandler(milvus_handler)



# print(debug_logger, qa_logger, rerank_logger, embed_logger, insert_logger, milvus_logger)
...
milvus_logger.propagate = False  # 关闭日志传播