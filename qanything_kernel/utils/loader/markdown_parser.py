...
def parse_markdown_mistune(file_path, doc_title=None, max_heading_depth=None):
    import mistune
        ...
        for idx, item in enumerate(v):
            if item['node_type'].startswith('Level'):
                if len(item['child_id_list']) == 0:  #是一个单独的标题，并且没有子节点，可能是markdown解析出现了问题
                    ...
                    doc = Document(page_content=tmp_content, metadata={'title_lst': [], 'has_table': False, 'title_level': k})
                    doc_lst.append(doc)
            if item['node_type'] == 'ContentNode':
                ...
                doc = Document(page_content=item['content'], metadata={'title_lst': title_lst, 'has_table': has_table, 'title_level': k})
                doc_lst.append(doc)
    return doc_lst
