...
# apt-get update
# apt install -y curl
# cd /workspace/QAnything/clash-for-linux-master || exit
# bash start.sh
...
# nohup python3 -u test_proxy.py > test_proxy.log 2>&1 &
...
echo "embedding和rerank服务将在GPU上运行"
...
PID1=$!
export CUDA_VISIBLE_DEVICES=$GPUID
nohup python3 -u qanything_kernel/dependent_server/embedding_server/embedding_server.py --use_gpu > /workspace/QAnything/logs/debug_logs/embedding_server.log 2>&1 &
PID2=$!
export CUDA_VISIBLE_DEVICES=$GPUID
nohup python3 -u qanything_kernel/dependent_server/pdf_parser_server/pdf_parser_server.py --use_gpu > /workspace/QAnything/logs/debug_logs/pdf_parser_server.log 2>&1 &
PID3=$!
export CUDA_VISIBLE_DEVICES=$GPUID
nohup python3 -u qanything_kernel/dependent_server/ocr_server/ocr_server.py --use_gpu > /workspace/QAnything/logs/debug_logs/ocr_server.log 2>&1 &
PID4=$!
...
echo "已耗时: ${elapsed} 秒."
user_ip=$USER_IP
echo "启动完成" #"请在[http://$user_ip:8777/qanything/]下访问前端服务来进行问答，如果前端报错，请在浏览器按F12以获取更多报错信息"
